#!/usr/bin/env python3
"""Prepare hold-blind top-percent NIMA supervision for ExBlur camellia."""
...
SCORES = Path(
    ...
    "camellia_exblur_nima_koniq_scores.json"
)
OUTPUT_DIR = Path(
    "/srv2/szha0669/blur_slam_exp/outputs/logs/"
    "exblur_camellia_nima_quantile_ablation_s1"
)
PERCENTS = (25, 20, 15, 10)


def main() -> None:
    base = json.loads(BASE_CONFIG.read_text())["exblur_camellia"]
    rows = sorted(
        json.loads(SCORES.read_text()),
        key=lambda row: (-float(row["nima_koniq"]), str(row["name"])),
    )
    if len(rows) != 34 or len({str(row["name"]) for row in rows}) != len(rows):
        raise RuntimeError("camellia NIMA score manifest is incomplete or duplicated")
    ...
    for percent in PERCENTS:
        ...
        scene = f"exblur_camellia_nimatop{percent}"
        cfg = dict(base)
        cfg.update(
            {

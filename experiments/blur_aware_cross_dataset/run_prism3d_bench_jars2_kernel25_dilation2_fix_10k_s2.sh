...
OUTPUT_ROOT="$ROOT/outputs/prism3d_bench_jars2_current_dilation2_fix_10k_s2"
...
[[ -x "$PYTHON" ]]
[[ -f "$CONFIG" ]]
for scene in prism3d_bench_turtle prism3d_jars2_turtle; do
...
run_scene 2 prism3d_bench_turtle &
pid_bench=$!
run_scene 3 prism3d_jars2_turtle &
pid_jars2=$!

status=0
wait "$pid_bench" || status=$?

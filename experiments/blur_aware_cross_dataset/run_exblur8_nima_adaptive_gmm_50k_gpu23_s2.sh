...
OUTPUT_ROOT="$BASE/outputs/learn2splat_legs_blur_evssm_exblur8_nima_adaptive_gmm_holdblind_50k_s2"
LOG_ROOT="$BASE/outputs/logs/learn2splat_legs_blur_evssm_exblur8_nima_adaptive_gmm_holdblind_50k_s2"
PREDECESSOR_STATUS="$BASE/outputs/logs/learn2splat_legs_blur_evssm_exblur_postbox_nima_quantile_10k_s1/status.tsv"

PHASE1_SCENE="exblur_stone_lantern"
GPU2_SCENES=(exblur_camellia exblur_jars2 exblur_sunflowers)
GPU3_SCENES=(exblur_bench exblur_dragon exblur_jars exblur_postbox)
mkdir -p "$OUTPUT_ROOT" "$LOG_ROOT"
cd "$REPO"

for scene in "$PHASE1_SCENE" "${GPU2_SCENES[@]}" "${GPU3_SCENES[@]}"; do
...
run_scene() {
  ...
  if CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" "$RUNNER" \
    ...
    local rc=$?
    printf '%s\tFAILED\t%s\tGPU%s\tRC%s\n' "$(date --iso-8601=seconds)" "$scene" "$gpu" "$rc" >> "$LOG_ROOT/status.tsv"
    return "$rc"
  fi
}
...
# Preserve the already-running quantile ablation and wait for its full queue.
while ! grep -q $'\tQUEUE_TERMINAL\t' "$PREDECESSOR_STATUS" 2>/dev/null; do
  printf '%s\tWAIT_PREDECESSOR\tpostbox_quantile_s1\n' "$(date --iso-8601=seconds)" >> "$LOG_ROOT/status.tsv"
  sleep 120
done

# Phase 1 must finish successfully before any other scene can start.
run_scene "$PHASE1_SCENE" 2

...
OUTPUT_ROOT="$BASE/outputs/learn2splat_legs_blur_evssm_exblur8_holdtrain_nimaw10_50k_s2"
LOG_ROOT="$BASE/outputs/logs/learn2splat_legs_blur_evssm_exblur8_holdtrain_nimaw10_50k_s2"
...
for scene, cfg in configs.items():
    ...
    assert cfg["evaluation_direct_supervision"] is True, scene
    assert cfg["exclude_evaluation_from_optimization"] is False, scene
    assert cfg["require_sharp_evaluation_targets"] is True, scene
print("EXBLUR_HOLDTRAIN_NIMAW10_PROTOCOL_GATE=PASS")
PY

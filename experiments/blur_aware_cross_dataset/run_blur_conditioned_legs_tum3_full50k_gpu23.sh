...
launch_scene() {
  ...
  echo "launched $scene pid=$pid physical_gpu=$gpu log=$log"
}

launch_scene "${SCENES[0]}" "${INITIAL_GPUS[0]}"
launch_scene "${SCENES[1]}" "${INITIAL_GPUS[1]}"
...
while ((${#PIDS[@]} > 0)); do
  ...
  set -e
  if [[ -z "$DONE_PID" ]]; then
    echo "wait -n returned without a PID" >&2
    exit 3
  fi
  ...
  if ((STATUS == 0)); then
    printf '%s\tCOMPLETE\t%s\tGPU%s\n' "$(date --iso-8601=seconds)" "$SCENE" "$GPU" \
      >> "$LOG_ROOT/status.tsv"
    echo "completed $SCENE on physical GPU $GPU"
    ...
    printf '%s\tFAILED(%s)\t%s\tGPU%s\n' \
      ...
      >> "$LOG_ROOT/status.tsv"
    echo "failed $SCENE on physical GPU $GPU with status $STATUS" >&2
    FAILED=1
  fi
done

if ((NEXT_SCENE < ${#SCENES[@]})); then
  echo "queue stopped before all scenes launched because an earlier scene failed" >&2
  exit 4
fi
exit "$FAILED"

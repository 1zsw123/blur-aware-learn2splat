...
RUN_TAG="${RUN_TAG:-local_objective_s1}"
LOCAL_OBJECTIVE="${LOCAL_OBJECTIVE:-on}"
...
GPUS=(2 3)
case "$LOCAL_OBJECTIVE" in
  on) LOCAL_OBJECTIVE_FLAG=(--legs-local-objective) ;;
  off) LOCAL_OBJECTIVE_FLAG=(--no-legs-local-objective) ;;
  *) echo "LOCAL_OBJECTIVE must be on or off" >&2; exit 2 ;;
esac
...
launch_scene() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" "$RUNNER" \
    ...
    --legs-blur-start-iter 500 \
    --legs-blur-ramp-iters 500 \
    "${LOCAL_OBJECTIVE_FLAG[@]}" \
    --coupled-dual-bpn \
    --skip-lpips \
    > "$log" 2>&1 &
  local pid=$!

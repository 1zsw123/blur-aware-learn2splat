    ...
    def _forward(self, x, y=None):
        ...
        if self.num_head > 1:
            ...
            scores = torch.matmul(q.permute(0, 2, 1, 3), k.permute(0, 2, 3, 1)) / (
                        (c // self.num_head) ** 0.5)  # [B*D, N, H*W, H*W]
        ...
        if postnorm:
            self.out = nn.Sequential(
                ...
                normalization(out_channels,
                              channels_per_group=channels_per_group) if not zero_final_layer else zero_module(
                    normalization(out_channels, channels_per_group=channels_per_group)),
                nn.SiLU(),
            )
        else:
            if self.downsample_factor is not None:

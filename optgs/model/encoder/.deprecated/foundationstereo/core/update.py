...
class BasicSelectiveMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=128, volume_dim=8, depth_head_dim=1):
        super().__init__()
        self.args = args
        ...
        if depth_head_dim > 1:
            # change the name to skip the pretrained weights
            self.depth_head = DispHead(hidden_dim, 256, output_dim=depth_head_dim)
        else:
    ...
    def forward(self, net, inp, corr, disp, att):
        ...
        if self.depth_head_dim > 1:
            delta_disp = self.depth_head(net[0])
        else:

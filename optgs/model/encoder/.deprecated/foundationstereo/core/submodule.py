...
def context_upsample(disp_low, up_weights, task='stereo'):
    """
    ...
    if task == 'depth':
        # since we predict inverse depth, we don't want to do zero padding
        disp_unfold = F.unfold(F.pad(disp_low.reshape(b,c,h,w), pad=(1,1,1,1), mode='replicate'),3,1,0).reshape(b,-1,h,w)
    else:

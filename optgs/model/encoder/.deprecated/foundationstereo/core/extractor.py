...
class Feature(nn.Module):
    def __init__(self, vit_type='vitl', no_freeze_mono=False):
        ...
        chans = [48, 96, 160, 304]
        self.chans = chans
        self.no_freeze_mono = no_freeze_mono
        ...
        self.dino = DepthAnythingFeature(encoder=vit_type)
        if not no_freeze_mono:
        ...
        if vit_type == 'vits':
            self.vit_proj = nn.Conv2d(32, 128, 1)
        elif vit_type == 'vitb':
            self.vit_proj = nn.Conv2d(64, 128, 1)
    ...
    def forward(self, x):
        ...
        if self.no_freeze_mono:
            ...
            with torch.no_grad():
                output = self.dino(x_in_)
        vit_feat = output['out']
        if self.vit_type in ['vits', 'vitb']:
            # TODO: or just copy the smaller features to 128

...
from .unimatch_geometry import coords_grid, generate_window_grid, normalize_coords
...
def group_correlation_softmax_depth(feature0, feature1,
                              intrinsics,
                              pose,
                              depth_candidates,
                              depth_from_argmax=False,
    ...
    with torch.amp.autocast('cuda', enabled=False):
        warped_feature1 = warp_with_pose_depth_candidates(feature1.float(), intrinsics, pose,
                                                        torch.exp(depth_candidates) if sample_log_depth else (1. / depth_candidates),
                                                        grid_sample_disable_cudnn=False,
                                                        )  # [B, C, D, H, W]

    channels_per_group = c // num_groups
    warped_feature1_reshape = warped_feature1.reshape(b, num_groups, channels_per_group, d, h, w)  # [B, G, C, D, H, W]
    feature0_reshape = feature0.reshape(b, num_groups, channels_per_group, 1, h, w)  # [B, G, C, 1, H, W]

    with torch.amp.autocast('cuda', enabled=False):
        correlation = (F.normalize(feature0_reshape.float(), dim=2) * F.normalize(warped_feature1_reshape.float(), dim=2)).sum(dim=2)  # [B, G, D, H, W]

    return correlation, warped_feature1
...
class CorrBlock:
    ...
    def corr(fmap1, fmap2):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            corr = torch.matmul(F.normalize(fmap1.float(), dim=1).transpose(1, 2), F.normalize(fmap2.float(), dim=1))

        corr = corr.view(batch, ht, wd, 1, ht, wd)

        # NOTE: normalize first for numerical stability
